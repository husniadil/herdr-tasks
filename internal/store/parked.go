...
// ListParked returns the actions still waiting in a project.
func (s *Store) ListParked(project string) ([]Parked, error) {
	rows, err := s.db.Query(
		`SELECT id, project, subject, verb, target, payload, state, COALESCE(reason, ''), created_at
		 FROM parked WHERE project = ? AND state = 'parked' ORDER BY id ASC`, project)
	if err != nil {
		return nil, wrap(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Project, &p.Subject, &p.Verb, &p.Target, &p.Payload,
			&p.State, &p.Reason, &p.CreatedAt); err != nil {
			return nil, wrap(err)
		}
		out = append(out, p)
	}
...
func (s *Store) GetParked(project, id string) (*Parked, error) {
	...
	err := s.db.QueryRow(
		...
		Scan(&p.ID, &p.Project, &p.Subject, &p.Verb, &p.Target, &p.Payload, &p.State, &p.Reason, &p.CreatedAt)
	if err != nil {
